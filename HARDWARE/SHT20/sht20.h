...
#define ST_SHT20_SUPPORT
#ifdef ST_SHT20_SUPPORT
typedef struct _SHT20_Date{
	float Temp;
	float Humi;
} SHT20_Date_T;
extern SHT20_Date_T gSht20Date;
#endif
typedef enum {
...
unsigned char SHT20_Init(void);//初始化	
unsigned char SHT20_SoftReset(void);//软件复位
float SHT20_GetTempPoll(void);//获取温度
float SHT20_GetHumiPoll(void);//获取湿度
