...
unsigned char RTC_Init(void)
{
	...
	RTC_Handler.Init.AsynchPrediv=32750; 	//时钟周期设置(有待观察,看是否跑慢了?)理论值：32767	
	if(HAL_RTC_Init(&RTC_Handler)!=HAL_OK)
		return 1;

	if(HAL_RTCEx_BKUPRead(&RTC_Handler,RTC_BKP_DR1)!=0X5050)//是否第一次配置
	{
		RTC_Set(2020,2,27,23,00,40); //设置日期和时间，2017年5月27日，17点02分0秒		 									  
...
{		 
	if(__HAL_RTC_ALARM_GET_FLAG(&RTC_Handler,RTC_FLAG_SEC)!=RESET)  //秒中断
	{
		...
		RTC_Get();				//更新时间   
		//HAL_GPIO_TogglePin(GPIOB,GPIO_PIN_6);													//LED1翻转
	}		
